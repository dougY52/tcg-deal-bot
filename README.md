...
Ein ausdrücklich beobachteter Wechsel von ausverkauft zu bestellbar wird als neuer Restock gemeldet, auch innerhalb von 24 Stunden. Die frühere Mindestdauer von sechs Stunden entfällt. Fehlende/fehlgeschlagene Abfragen zählen weiterhin nicht als Ausverkauf; unveränderte Angebote werden nicht erneut gepostet.

### Aktueller Angebotsmodus: weitere versiegelte Produkte und Preisverlauf

`market.expanded_products = true` erweitert die Prüfung auf Top-Trainer-Boxen/ETBs, Tins, Mini-Tins, Booster-Bundles, Kollektionen, Blister und Boxen mit Karteninhalt. Leere Aufbewahrungsboxen, Zubehör, Einzelkarten, Mystery-Angebote und unklare Bulk-Angebote bleiben ausgeschlossen. Produktformat, Sprache und Motiv/Variante werden getrennt verglichen. Bei fehlender Packzahl wird für diese Formate keine erfundene Anzahl angezeigt. Pokémon bleibt ausschließlich deutsch.

`market.price_context_mode = true` ersetzt die frühere reine Deal-Freigabe durch bewertete Verfügbarkeitshinweise. Es muss kein anderer Händler aktuell liefern können. Vorrang haben datierte Normalpreis-/UVP-Belege, dann hinterlegte externe Preisbelege (`price_references`, `kind: market_reference`), danach tatsächliche Internet-Händlerbeobachtungen der letzten 14 Tage und schließlich aktuelle Vergleichsangebote. Historische Vergleiche verwenden den Median der niedrigsten beobachteten Preise je Händler und Tag; Beobachtungen unter sechs Stunden werden nicht als mehrtägiger Verlauf dargestellt. Quelle und Beobachtungsdatum stehen in Discord. Ausverkaufte Preise werden nicht nachträglich als bestellbare historische Angebote gewertet.
